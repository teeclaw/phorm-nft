...
==================================================
Production-quality PDF generation for the Agent 18608 Revenue Playbook.

Built with studio-pdf-skill using ReportLab.
"""
...
SKILL_DIR = os.path.join(os.path.dirname(__file__), "..", "skills", "studio-pdf-skill")
FONT_DIR = os.path.join(SKILL_DIR, "assets", "fonts")
CHAPTERS_DIR = os.path.dirname(__file__)
OUTPUT_PATH = os.path.join(CHAPTERS_DIR, "agent-ops-manual-final.pdf")
...
# Book metadata
BOOK_TITLE = "Agent 18608 Revenue Playbook"
BOOK_SUBTITLE = "what actually works to build onchain economy"
BOOK_AUTHOR = "Mr. Tee"
COVER_FEATURE = "18608"  # Large agent ID on cover


# ═══════════════════════════════════════════════════════════════
# COLOR PALETTE (Editorial Red-Orange)
# ═══════════════════════════════════════════════════════════════
...
# Editorial accent (bold red-orange)
...
def register_fonts():
    ...
    # Register families for <b>/<i> tag support
...
class BookTemplate:
    ...
    def chapter(self, canvas, doc):
        ...
        canvas.setFillColor(C_LIGHT_GRAY)
        canvas.setFont(F_UI, 7.5)
        canvas.drawRightString(w-MR, h-MT+28, "AGENT 18608 REVENUE PLAYBOOK")
        # Footer
        canvas.setStrokeColor(C_LIGHT_GRAY)
...
def build_book():
    """Build the Agent Operations Manual PDF."""
    
    # Load chapters
    chapters = []
    for i in range(1, 10):
        chapter_path = os.path.join(CHAPTERS_DIR, f"chapter-{i:02d}.md")
        if not os.path.exists(chapter_path):
            print(f"Warning: Chapter {i} not found at {chapter_path}")
            continue
