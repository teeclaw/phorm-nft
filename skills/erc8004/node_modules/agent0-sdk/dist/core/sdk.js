...
import { DEFAULT_REGISTRIES, DEFAULT_SUBGRAPH_URLS, IDENTITY_REGISTRY_ABI, } from './contracts.js';
/**
...
export class SDK {
    constructor(config) {
        ...
        this._feedbackManager = new FeedbackManager(this._chainClient, this._ipfsClient, undefined, // reputationRegistryAddress - will be set lazily
        ...
        this._feedbackManager.setSubgraphClientGetter((chainId) => this.getSubgraphClient(chainId), this._chainId);
    }
    /**
     * Initialize IPFS client based on configuration
     */
    _initializeIpfsClient(config) {
        if (!config.ipfs) {
    ...
    async loadAgent(agentId) {
        ...
        let agentURI;
        try {
            agentURI = await this._chainClient.readContract({
    ...
    async isAgentOwner(agentId, address) {
        ...
        const owner = await this._chainClient.readContract({
            address: this.identityRegistryAddress(),
            abi: IDENTITY_REGISTRY_ABI,
    ...
    async getAgentOwner(agentId) {
        const { tokenId } = parseAgentId(agentId);
        return await this._chainClient.readContract({
            address: this.identityRegistryAddress(),
            abi: IDENTITY_REGISTRY_ABI,
            functionName: 'ownerOf',
            args: [BigInt(tokenId)],
     ...
     * Give feedback
     */
    async giveFeedback(agentId, value, tag1, tag2, endpoint, feedbackFile) {
    ...
    async searchFeedback(filters, options = {}) {
        ...
        const hasAnyFilter = (agents?.length ?? 0) > 0 ||
        ...
        if (!hasAnyFilter) {
            throw new Error('searchFeedback requires at least one filter (agentId/agents/reviewers/tags/capabilities/skills/tasks/names/minValue/maxValue).');
        }
        const params = {
            agents,
            tags: filters.tags,
     ...
     */
    async appendResponse(agentId, clientAddress, feedbackIndex, response) {
        // Update feedback manager with registries
        this._feedbackManager.setReputationRegistryAddress(this.reputationRegistryAddress());
     ...
     */
    async _loadRegistrationFile(tokenUri) {
        try {
            ...
            return this._transformRegistrationFile(rawData);
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error);
     ...
     */
    _transformRegistrationFile(rawData) {
        ...
        const trustModels = Array.isArray(rawData.supportedTrust)
        ...
        return {
            ...
            endpoints,
            trustModels,
            owners: Array.isArray(rawData.owners) ? rawData.owners.filter((o) => typeof o === 'string') : [],
            operators: Array.isArray(rawData.operators) ? rawData.operators.filter((o) => typeof o === 'string') : [],
            ...
            x402support: typeof rawData.x402support === 'boolean'
                ? rawData.x402support
     ...
     */
    _transformEndpointLegacy(ep, rawData) {
