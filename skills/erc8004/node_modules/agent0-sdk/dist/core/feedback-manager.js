...
const GIVE_FEEDBACK_GAS_LIMIT = 300000n;
/**
 * Manages feedback operations for the Agent0 SDK
 */
export class FeedbackManager {
    constructor(chainClient, ipfsClient, reputationRegistryAddress, identityRegistryAddress, subgraphClient) {
     ...
     */
    setSubgraphClientGetter(getter, defaultChainId) {
     ...
     */
    setReputationRegistryAddress(address) {
     ...
     */
    async giveFeedback(agentId, value, tag1, tag2, endpoint, feedbackFile, idem) {
        // Parse agent ID
        ...
        if (this.chainClient.chainId !== agentChainId) {
            throw new Error(`Chain mismatch for giveFeedback: agentId=${agentId} targets chainId=${agentChainId}, ` +
        ...
        try {
            ...
            const lastIndex = await this.chainClient.readContract({
        ...
        if (this.ipfsClient && hasOffchainFile) {
            ...
            try {
                ...
                const createdAt = typeof feedbackFile?.createdAt === 'string'
                    ? feedbackFile.createdAt
        ...
        let txHash;
        try {
            const txOptions = { gasLimit: GIVE_FEEDBACK_GAS_LIMIT };
        ...
        return new TransactionHandle(txHash, this.chainClient, async () => {
            ...
            return {
                ...
                tags: [tag1OnChain || undefined, tag2OnChain || undefined].filter(Boolean),
     ...
     */
    async _getFeedbackFromBlockchain(agentId, clientAddress, feedbackIndex) {
        if (!this.reputationRegistryAddress) {
            throw new Error('Reputation registry not available');
        }
        ...
        try {
            ...
            const tags = [tag1, tag2].filter((t) => t && t !== '');
            ...
            if ((!endpoint || endpoint === '') && fileURI && this.ipfsClient && fileURI.startsWith('ipfs://')) {
                try {
                    ...
                    const file = await this.ipfsClient.getJson(cid);
     ...
     */
    _mapSubgraphFeedbackToModel(feedbackData, agentId, clientAddress, feedbackIndex) {
        const feedbackFile = feedbackData.feedbackFile || {};
        // Map responses
        const responsesData = feedbackData.responses || [];
        const answers = responsesData.map((resp) => ({
        ...
        const id = [agentId, clientAddress, feedbackIndex];
        return {
            id,
            agentId,
            reviewer: clientAddress,
            ...
            tags,
            endpoint: typeof feedbackData.endpoint === 'string'
    ...
    async getReputationSummary(agentId, tag1, tag2) {
        ...
        if (subgraphClient) {
            try {
                ...
                if (tag1 || tag2) {
                    filteredFeedbacks = feedbacksData.filter((fb) => {
                ...
                const validFeedbacks = filteredFeedbacks.filter((fb) => !fb.isRevoked);
        ...
        if (chainId !== undefined && this.defaultChainId !== undefined && chainId !== this.defaultChainId) {
            throw new Error(`Blockchain reputation summary not supported for chain ${chainId}. ` +
                `SDK is configured for chain ${this.defaultChainId}. ` +
                `Use subgraph-based summary instead.`);
        }
        try {
            const clients = await this.chainClient.readContract({
                address: this.reputationRegistryAddress,
                abi: REPUTATION_REGISTRY_ABI,
