...
export class SubgraphClient {
    ...
    async query(query, variables) {
        try {
            ...
            if ((msg.includes('Cannot query field "hasOASF"') || msg.includes('has no field `hasOASF`')) &&
                query.includes('hasOASF')) {
                const q2 = query.split('hasOASF').join('oasfEndpoint');
                const data2 = await this.client.request(q2, variables || {});
                return data2;
            }
            if ((msg.includes('Cannot query field "x402Support"') || msg.includes('has no field `x402Support`')) &&
    ...
    async getAgents(options = {}) {
        ...
        if (where.owner_in)
            supportedWhere.owner_in = where.owner_in;
        if (where.operators_contains)
            supportedWhere.operators_contains = where.operators_contains;
        if (where.agentURI)
    ...
    `;
        ...
        try {
            const data = await this.query(query, variables);
            return (data.agents || []).map((a) => this._transformAgent(a));
        }
        catch (e) {
            ...
            const mentionsHasOASF = msg.includes('hasOASF') &&
            ...
            if (mentionsHasOASF && variables.where) {
                    ...
                    for (const [k, v] of Object.entries(node)) {
                        if (k === 'registrationFile_' && v && typeof v === 'object') {
                            ...
                            if (Object.prototype.hasOwnProperty.call(rf, 'hasOASF')) {
                                const want = Boolean(rf.hasOASF);
                                delete rf.hasOASF;
                                // Best-effort fallback: older subgraphs commonly have `oasfEndpoint`.
                                if (want)
                                    rf.oasfEndpoint_not = null;
                                else
                                    rf.oasfEndpoint = null;
                            }
                            out[k] = rewrite(rf);
                        }
                        else {
                ...
                const data2 = await this.query(query, variables2);
                return (data2.agents || []).map((a) => this._transformAgent(a));
            }
            throw e;
        }
    }
    async queryAgentMetadata(where, first, skip) {
        const query = `
    ...
    `;
        try {
            const data = await this.query(query, { where, first, skip });
        ...
        `;
                const data2 = await this.query(q2, { where, first, skip });
                return data2.agentMetadata_collection || [];
            }
            throw e;
        }
    }
    async queryFeedbacks(where, first, skip, orderBy = 'createdAt', orderDirection = 'desc') {
        const query = `
    ...
    async queryFeedbackResponses(where, first, skip) {
        const query = `
    ...
    async searchAgents(params, first = 100, skip = 0) {
        ...
        if (params.active !== undefined || params.hasMCP !== undefined || params.hasA2A !== undefined ||
            ...
            if (params.ensContains)
                registrationFileFilters.ens_contains_nocase = params.ensContains;
    ...
    `;
        ...
        try {
            const result = await this.query(queryWithEndpoint);
      ...
      `;
            const result = await this.query(queryWithoutEndpoint);
            return result.feedbacks || [];
        }
    }
}
