...
export class AgentIndexer {
    constructor(subgraphClient, subgraphUrlOverrides, defaultChainId) {
        ...
        this.subgraphUrlOverrides = subgraphUrlOverrides;
     ...
     */
    async searchAgents(params = {}, options = {}) {
    ...
    _parseSort(sort, keywordPresent) {
    ...
    async _fetchAllAgentsV2(client, where, orderBy, orderDirection) {
        const out = [];
        for (let skip = 0;; skip += AgentIndexer.BATCH_SIZE) {
            ...
            out.push(...page);
            if (page.length < AgentIndexer.BATCH_SIZE)
                break;
        }
        return out;
    }
    _normalizeAgentIds(filters, chains) {
        var _a;
        ...
        for (const id of ids) {
            ...
            if (s.includes(':')) {
                ...
                if (!Number.isFinite(chainId))
                    continue;
                (byChain[chainId] || (byChain[chainId] = [])).push(s);
            }
            else {
                if (chains.length !== 1) {
                    throw new Error('agentIds without chain prefix are only allowed when searching exactly one chain.');
                }
                (byChain[_a = chains[0]] || (byChain[_a] = [])).push(`${chains[0]}:${s}`);
            }
        }
        return byChain;
    }
    _toUnixSeconds(input) {
    ...
    _buildWhereV2(filters, idsForChain) {
        const and = [];
        const base = {};
        ...
        if (fb && (fb.hasFeedback || fb.hasNoFeedback)) {
            const hasThreshold = fb.minCount !== undefined || fb.maxCount !== undefined || fb.minValue !== undefined || fb.maxValue !== undefined;
            const hasAnyConstraint = fb.hasResponse === true ||
        ...
        if (filters.registeredAtFrom !== undefined)
            base.createdAt_gte = this._toUnixSeconds(filters.registeredAtFrom);
        if (filters.registeredAtTo !== undefined)
            base.createdAt_lte = this._toUnixSeconds(filters.registeredAtTo);
        if (filters.updatedAtFrom !== undefined)
            base.updatedAt_gte = this._toUnixSeconds(filters.updatedAtFrom);
        if (filters.updatedAtTo !== undefined)
            base.updatedAt_lte = this._toUnixSeconds(filters.updatedAtTo);
        const rf = {};
        if (filters.name)
        ...
        if (filters.hasMCP !== undefined)
            rf[filters.hasMCP ? 'mcpEndpoint_not' : 'mcpEndpoint'] = null;
        if (filters.hasA2A !== undefined)
            rf[filters.hasA2A ? 'a2aEndpoint_not' : 'a2aEndpoint'] = null;
        if (filters.hasWeb !== undefined)
            rf[filters.hasWeb ? 'webEndpoint_not' : 'webEndpoint'] = null;
        // Exact semantics: true iff (oasfSkills OR oasfDomains) non-empty. Implemented by subgraph derived field.
        if (filters.hasOASF !== undefined)
            rf.hasOASF = filters.hasOASF;
    ...
    _compareAgents(a, b, field, direction) {
    ...
    async _prefilterByMetadata(filters, chains) {
        ...
        const perChain = await Promise.all(chains.map(async (chainId) => {
            const client = this.defaultChainId !== undefined && chainId === this.defaultChainId ? this.subgraphClient : this._getSubgraphClientForChain(chainId);
            if (!client)
                return { chainId, ids: [] };
            const ids = [];
    ...
    async _prefilterByFeedback(filters, chains, candidateIdsByChain) {
        const fb = filters.feedback;
        if (!fb)
            return {};
        const hasThreshold = fb.minCount !== undefined || fb.maxCount !== undefined || fb.minValue !== undefined || fb.maxValue !== undefined;
        ...
        if (fb.hasNoFeedback) {
            ...
            if (!anyCandidates)
                throw new Error('feedback.hasNoFeedback requires a pre-filtered candidate set (e.g. agentIds or keyword).');
        }
        const first = 1000;
        const statsById = {};
        const matchedAgentsByChain = {};
        await Promise.all(chains.map(async (chainId) => {
            ...
            if (fb.endpoint)
                base.endpoint_contains_nocase = fb.endpoint;
            ...
            for (let skip = 0;; skip += first) {
                ...
                for (const r of rows) {
                    if (!r?.agent?.id)
                        continue;
                    if (fb.hasResponse && !(r.responses && r.responses.length > 0))
                        continue;
                    const agentId = r.agent.id;
                    const v = Number(r.value);
                    if (!Number.isFinite(v))
                        continue;
                    const s = (statsById[agentId] || (statsById[agentId] = { sum: 0, count: 0 }));
                    s.sum += v;
                    s.count += 1;
                    (matchedAgentsByChain[chainId] || (matchedAgentsByChain[chainId] = new Set())).add(agentId);
                }
                if (rows.length < first)
                    break;
            }
        ...
        const passes = (st) => {
            if (fb.minCount !== undefined && st.count < fb.minCount)
                return false;
            if (fb.maxCount !== undefined && st.count > fb.maxCount)
                return false;
            if (fb.minValue !== undefined && st.avg < fb.minValue)
                return false;
            if (fb.maxValue !== undefined && st.avg > fb.maxValue)
                return false;
            return true;
        };
        const allowByChain = {};
        for (const chainId of chains) {
            const matched = matchedAgentsByChain[chainId] || new Set();
            const candidates = candidateIdsByChain?.[chainId];
            if (fb.hasNoFeedback) {
    ...
    async _searchUnifiedNoKeyword(filters, options) {
        ...
        const fetchChain = async (chainId) => {
            try {
                const client = this.defaultChainId !== undefined && chainId === this.defaultChainId ? this.subgraphClient : this._getSubgraphClientForChain(chainId);
                if (!client)
                    return { chainId, status: 'unavailable', items: [] };
    ...
    async _searchUnifiedWithKeyword(filters, options) {
        var _a;
        ...
        for (const r of filtered) {
            (byChain[_a = r.chainId] || (byChain[_a] = [])).push(r.agentId);
    ...
    _filterAgents(agents, params) {
        const { name, description, hasMCP, hasA2A, mcpContains, a2aContains, webContains, ensContains, didContains, walletAddress, supportedTrust, a2aSkills, mcpTools, mcpPrompts, mcpResources, active, x402support, chains, } = params;
     ...
     */
    _getSubgraphClientForChain(chainId) {
