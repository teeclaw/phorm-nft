 ...
 *
 * Env:
 *   PRIVATE_KEY       — Required for write operations (register, set-metadata, unset-wallet) unless USE_KMS=true
 *   USE_KMS           — Set to 'true' to use GCP Cloud KMS HSM signer (0x1Af5f519DC738aC0f3B58B19A4bB8A8441937e78)
...
function getSigner() {
  ...
  // Use GCP KMS if USE_KMS=true or if PRIVATE_KEY is not set
  const useKms = process.env.USE_KMS === 'true' || !process.env.PRIVATE_KEY;
  
  if (useKms) {
    console.log("  Using GCP Cloud KMS HSM signer (0x1Af5f519DC738aC0f3B58B19A4bB8A8441937e78)");
    const signer = new KmsSigner(provider);
    return { config, signer };
  }
  
  const pk = process.env.PRIVATE_KEY;
  if (!pk) die("PRIVATE_KEY env var not set. Add it to your OpenClaw config under skills.entries.zscore.env or set USE_KMS=true");
...
async function main() {
  ...
  if (args.length === 0) {
    ...
    console.log("");
    console.log("Env:");
    console.log("  PRIVATE_KEY  Required for write operations (unless USE_KMS=true)");
    console.log("  USE_KMS      Set to 'true' to use GCP Cloud KMS HSM signer");
