 ...
 */
function recoverV(digest: Buffer, r: bigint, s: bigint, expectedAddress: string): { r: string; s: string; v: number } {
 ...
 */
export async function kmsSign(digest: Buffer): Promise<{ r: string; s: string; v: number }> {
...
export class KmsSigner extends ethers.AbstractSigner {
  ...
  constructor(provider: ethers.Provider | null = null) {
    super(provider);
  ...
  async signDigest(digest: string | Buffer): Promise<ethers.Signature> {
  ...
  async signMessage(message: string | Uint8Array): Promise<string> {
    ...
    const sig = await this.signDigest(Buffer.from(digest));
