...
- **Agent ID 14482 is the current active ID** 🔒 PRIMARY (was never burned)
- **Agent ID 16919 was burned** — all references removed
...
## openclaw-8004 Skill — Full Test Session
- Installed `@basecred/openclaw-8004` (v1.3.0) via npm
- SKILL.md located at `node_modules/@basecred/openclaw-8004/SKILL.md`

### Test Results (agents 16926, 16928 — both burned after testing)

**Register test (16926 — HTTP URL):**
- Minted successfully but metadata unreachable (dummy URL)
- `tokenURI` returned dead HTTP link → no metadata for consumers
- Lesson: SDK doesn't validate URI before tx — gas wasted on dead links

**Register test (16928 — base64 data URI) ✅:**
- Built JSON → base64 encoded → `data:application/json;base64,...`
- Metadata fully readable on-chain via `tokenURI()` — no external dependency
- This is the correct approach for fully on-chain registration

**Update test (16928) ✅:**
- Read `tokenURI` via viem → decoded base64 → modified JSON (added image) → re-encoded → submitted
...
- Setting `active: false` in raw JSON didn't work — SDK overrides with internal state
- Must call `agent.setActive(false)` before `registerHTTP()`

### SDK Gotchas Documented in SKILL.md
1. `addEndpoint()` doesn't exist — use `setMetadata()` for custom services
2. `addSkill()`/`addDomain()` strict OASF validation — pass `false` to skip
3. `loadAgent()` fails when URI unreachable — use `createAgent()` + `registrationFile.agentId` instead
4. SDK overrides `active` field — must use `agent.setActive()`, not raw JSON
5. SDK return object missing custom fields (e.g. `image`) — verify via raw `tokenURI()` read

### SKILL.md Status
- **Register section:** 🔒 Locked (fully on-chain base64 data URI flow)
- **Update section:** 🔒 Locked (read→decode→modify→re-encode→submit flow)
...
- **Self-feedback blocked:** Contract reverts "Self-feedback not allowed" — same wallet can't rate own agent
- **Cross-wallet works:** Successfully rated 8453:14428 (scores 85, 90) and 8453:16934 (scores 50, 100)
- **Auto-generated feedback:** Read on-chain metadata → assess → generate score + tags → preview → confirm
- **Manual feedback:** Interactive flow — ask agent ID, score, tags → preview → confirm
...
- 3-layer max: Main Menu → Sub-menu → Confirmation
- All sub-menus: Update (From On-chain/Manual), Search (My Agent/Other), Feedback (Auto/Manual), Register (Pre-filled/Manual/Dummy)
- Confirmation buttons on ALL drafts: [✅ Submit] [✏️ Modify] [❌ Cancel] [⬅️ Back]
- Every result/error screen has `8004:menu` back button — no dead-ends
...
## Burned Test Agents
- 16926 (HTTP URL test) — burned
- 16928 (base64 data URI test) — burned
- 16930 (full register test) — burned
- 16932 (PhantomBot Zeta) — burned

- 14499 (early mint, no metadata) — burned (deactivated first, then transferred to 0xdEaD)
...
## Multi-Agent Detection — Config-First Approach (DECIDED)
...
- Agent IDs are public on-chain data — belong in TOOLS.md, NOT .env
...
- Rewrote entire SKILL.md — all 30 contract calls now use direct viem
- Zero agent0-sdk dependency
- Key operations mapped:
  - Register: `writeContract('register', [dataURI])` + parse Transfer event for agentId
  - Update: `writeContract('setAgentURI', [id, dataURI])` — no more registrationFile.agentId hack
  - Transfer/Burn: `writeContract('transferFrom', [from, to, id])` — SDK broke on data URIs
  - Feedback: `writeContract('giveFeedback', [...])` direct
  - Search: `getClients()` → `getSummary()` (empty clientAddresses array REVERTS)
  - Read: `readContract('tokenURI', [id])` — SDK loadAgent() broke on data URIs

## SDK Gotchas Discovered (viem migration)
- `sdk.transferAgent()` fails on data URI agents — tries to loadAgent() first, rejects data: URIs
- `getSummary()` reverts with "clientAddresses required" if passed empty array — must call `getClients()` first
- `readAllFeedback()` same — needs actual client addresses
...
- Transfer sub-menu now has [🔄 Transfer to Address] and [🔥 Burn Agent] buttons

## Button Flow Test
- Agent picker → select 14499 → main menu → Transfer/Burn → Burn → confirm → executed ✅
